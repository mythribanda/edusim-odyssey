...
type Series = { title: string; color: string; unit: string; get: (s: LawSample) => number };

export function LawGraphPanel({ samples, series }: { samples: LawSample[]; series: Series[] }) {
  const T = Math.max(samples.length ? samples[samples.length - 1].t : 1, 0.5);
  return (
    <div className="glass-strong rounded-3xl p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
      {series.map((s) => (
        <Graph key={s.title} samples={samples} T={T} {...s} />
      ))}
    </div>
  );
}
...
}: Series & { samples: LawSample[]; T: number }) {
